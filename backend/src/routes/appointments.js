...
router.patch('/:id/confirm', authenticate, authorize('THERAPIST', 'ADMIN'), async (req, res, next) => {
  try {
    const { id } = req.params;
...
router.patch('/:id/complete', authenticate, authorize('THERAPIST'), async (req, res, next) => {
  try {
    const { id } = req.params;
